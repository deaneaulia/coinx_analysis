WITH
  ...
  FROM raw_data
  GROUP BY user_id, token_category
),
...
monthly_activity AS (
  ...
  FROM raw_data
  GROUP BY user_id, token_category, activity_month
),
...
cohort_analysis AS (
  SELECT 
    ...
    cb.user_id
  FROM cohort_base cb
  JOIN monthly_activity ma ON cb.user_id = ma.user_id AND cb.token_category = ma.token_category
),
...
cohort_summary AS (
  ...
  GROUP BY cohort_month, token_category, month_number
),
...
cohort_sizes AS (
  ...
  FROM cohort_base
  GROUP BY cohort_month, token_category
)
...
JOIN cohort_sizes cs ON ca.cohort_month = cs.cohort_month AND ca.token_category = cs.token_category
WHERE ca.month_number >= 0
ORDER BY cs.cohort_month, cs.token_category, ca.month_number;
